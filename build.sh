...
if [ ! -f config.status ]; then
    ...
    CFLAGS="-s USE_ZLIB=1 -s USE_SDL=2 -s USE_SDL_TTF=2 -s USE_SDL_MIXER=2 -s USE_SDL_GFX=2 -s USE_SDL_IMAGE=2 -pthread" \
    LDFLAGS="-s USE_FREETYPE=1 -s USE_HARFBUZZ=1 -s LEGACY_GL_EMULATION=1 -s INITIAL_MEMORY=300MB -s ALLOW_MEMORY_GROWTH=1 -pthread -s PTHREAD_POOL_SIZE=3"
fi
...
-pthread -s PTHREAD_POOL_SIZE=3 -s LEGACY_GL_EMULATION=1 -s INITIAL_MEMORY=300MB -s ALLOW_MEMORY_GROWTH=1 -fexceptions -s DISABLE_EXCEPTION_CATCHING=0 \
